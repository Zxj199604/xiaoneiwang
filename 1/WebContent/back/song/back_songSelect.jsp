...
window.location.href="SongSerlvet?method=3&id="+id;
}
}
</script>
</head>
...
if(pagination.getRecordSize()<=0){
    ...
    %>
    <a href="back_songSelect.jsp?typeId=<%=songTypeForm.getId()%>"> [<%=songTypeForm.getTypeName()%>]</a>&nbsp;
<%} %>
    </div>
	</td>
	</tr>
    ...
    <tr>
     ...
     <%for(int i=0;i<list.size();i++){ 
         ...
         <tr bgcolor="#FFFFFF">
			 ...
			 <a href="back_songRestoreSelect.jsp?id=<%=songForm.getId()%>" title="可以查看回复内容"> <%=songForm.getSongName()%></a>
