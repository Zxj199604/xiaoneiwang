...
if(confirm("确定要删除此文章及其相关的评论吗？")){
window.location.href="ArticleServlet?method=8&id="+id;
}
}
</script>
</head>
...
List list=null;
if(str==null){
	list=articleDao.queryArticle1(typeId,consumerForm1.getId());
...
if(pagination.getRecordSize()<=0){
    ...
    %>
    <a href="back_ArticleSelect1.jsp?typeId=<%=articleTypeForm.getId()%>"> [<%=articleTypeForm.getTypeName()%>]</a>&nbsp;
    <%}%>
    </div>
	</td>
	</tr>
     ...
     <%for(int i=0;i<list.size();i++){ 
         ...
         <tr bgcolor="#FFFFFF">
			 ...
			 <%=articleForm.getTitle()%>
              ...
              <td><div align="center"><a href="javascript:deleteForm('<%=articleForm.getId()%>')">删除</a></div></td>
            </tr>
         <%} %>
    </table>
     <%if(request.getParameter("typeId")==null){ %>
